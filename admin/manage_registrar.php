...
$tbl_user_accounts = array();
while ($row = $qry->fetch_assoc()) {
	$tbl_user_accounts[] = $row['user_id'];
}
?>
	...
	<form action="" id="ar-frm">
		...
		<div class="form-group">
			...
			<select name="user_id[]" id="user_id" multiple="multiple" class="custom-select custom-select-sm select2">
				<option value=""></option>
				<?php
				$uqry = $conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM tbl_user_accounts where type = 2 order by concat(lastname,', ',firstname,' ',middlename) asc");
				while ($row = $uqry->fetch_assoc()) :
				?>
					<option value="<?php echo $row['id'] ?>" <?php echo count($tbl_user_accounts) > 0 && in_array($row['id'], $tbl_user_accounts) ? "selected" : '' ?>><?php echo ucwords($row['name']) ?></option>
				<?php endwhile; ?>
			</select>
		</div>
	</form>
